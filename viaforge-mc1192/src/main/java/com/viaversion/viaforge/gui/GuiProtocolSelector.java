...
public class GuiProtocolSelector extends Screen {
    ...
    public void init() {
        super.init();
        addRenderableWidget(new Button(5, height - 25, 20, 20, Component.literal("<-"), b -> minecraft.setScreen(parent)));
        if (!this.simple) {
            addRenderableWidget(new Button(width - 105, 5, 100, 20, Component.literal("Create dump"), b -> {
                try {
            ...
            addRenderableWidget(new Button(width - 105, height - 25, 100, 20, Component.literal("Reload configs"), b -> Via.getManager().getConfigurationProvider().reloadConfigs()));
        }
    ...
    public void render(PoseStack matrices, int p_230430_2_, int p_230430_3_, float p_230430_4_) {
        ...
        drawString(matrices, font, status != null ? status : "Discord: http://discord.gg/viaversion", 3, 3, -1);
    }
